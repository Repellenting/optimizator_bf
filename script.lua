...
local function ultraOptimization()
    if not cfg.ultraOpt then return end
...
local function disableAllGui()
    if not cfg.disableGui then return end
    task.spawn(function()
        local success = false
...
print("INV System Loaded | FPS:", cfg.fps)
